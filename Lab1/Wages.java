...
class EmployeeList {
   ...
   public Employee find(String name) {
      for (int i = 0; i < employeeList.size(); ++i) {
         if (employeeList.get(i).name.equals(name)) return employeeList.get(i);
      }
   ...
   public void setHours(String name, double hours) {
      this.find(name).setHours(hours);
   }

   public void setSalary(String name, double salary) {
      this.find(name).setSalary(salary);
   }

   public void setSales(String name, double sales) {
      this.find(name).setSales(sales);
   }
   ...
   public double payroll() {
      ...
      for (int i = 0; i < employeeList.size(); ++i) {
         sumTotal += employeeList.get(i).computePay();
      }
